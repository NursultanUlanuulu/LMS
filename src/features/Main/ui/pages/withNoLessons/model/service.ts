...
import { getPendingsList, getUnrecordedList } from '@/features/Main/store/actions'
...
import { selectUnrecorded } from '@/features/Main/store/selector';
import { selectSubjects } from '@/features/Subjects/store/selector';

const listService = () => {
    const dispatch = useAppDispatch()
    const headerLinks = ["ФИО", "Номер телефона", "Предмет", "Дни", "Время", "Преподаватель", "Уровень"]
    const list = useAppSelector(selectUnrecorded)
    ...
    useEffect(() => {
        dispatch(getUnrecordedList(filter))
    }, [filter])
    return {
